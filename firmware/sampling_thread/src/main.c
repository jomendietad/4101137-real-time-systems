/*
 * SoilSense Control — week 3: the superloop with its first thread.
 * Sampling runs in a preemptive kernel thread fed by the tick ISR; everything
 * else is still the week-2 superloop in main.
 *
...
/* Sampling tick: the ISR queues the release time and returns; the sampling
 * thread wakes on it. Ticks still pile up here if the thread falls behind. */
...
/* Blocked (zero CPU) until the ISR queues a tick; above main's priority, so it
 * preempts whatever the loop is doing. Control stays in the loop: every 10th
 * sample raises a flag there, exactly like the old tick. */
...
{
	init_hw();
	printk("SoilSense Control — sampling-thread build (%s)\n", CONFIG_BOARD);
	printk("type 'help'\n");
